<h1 align="center">Velofy</h1>

<p align="center"><strong>AI Native builders out of Gurgaon, India.</strong></p>

<p align="center">Open Source. by Nature.</p>

---

We build software that is AI native from the first commit: tools, frameworks, and harnesses designed for a world where AI agents write, read, and run code alongside people.

Many of our open source projects are in active development. Here is what is already out:

## Summit.js

[**Summit.js**](https://github.com/velofy/summit) is the open source, AI Agent Native JavaScript framework for composing behavior directly in your HTML. Drop in one script and go. No build step, no virtual DOM, no `eval`.

- **HTML-first and local.** Behavior lives on the element it affects, so an agent edits one place and sees the result.
- **Safe by construction.** Expressions are interpreted, never `eval`ed, so generated markup runs under a strict CSP.
- **Built to be read by machines.** `llms.txt`, full-corpus markdown, and a drop-in `AGENTS.md` brief for your own agent.
- **A UI library, included.** Accessible, token-themed components in about 16KB gzipped.

Docs: [velofy.github.io/summit](https://velofy.github.io/summit/)

## More on the way

We are building more AI native open source projects, from agent harnesses to developer tools. Watch this org, more repos are opening up soon.

---

<p align="center">Made with intent in Gurgaon, India 🇮🇳</p>
